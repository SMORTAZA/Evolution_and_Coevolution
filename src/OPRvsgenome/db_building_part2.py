	...
	for file in files :
		...
		for line in filin :
			...
			if contig not in genomic_hit[genome] :
				genomic_hit[genome][contig] = (contig_length, region, \
					region_length)
		filin.close()
	return genomic_hit
...
def G_contig(rep_resume1_files) :
	...
	for genome in dico_region :
		...
		for contig in dico_region[genome] :
			...
			#si la region trouvée est inf à 50kb, alors on considere que c'est 
			#une region d'un gene
			if region_length < 50 :
...
def dico_info_fasta(rep_fasta_files) :
	...
	#print dico_fasta["GCA_002335675.1_C.eustigma_genome_v1.0_protein.faa"],
	#print ["GAX86705.1"]
...
def dico_G_regions(G_regions_file) :
	...
	for line in filin :
		...
		if '>' in line :
			...
			dico_G[species][region_name] = (int(line.split('\t')[1]), \
				int(line.split('\t')[2][:-1]))
	filin.close()
	#verif dico
...
def blast_part2(cmd_blast, type_ome, regions_file, rep_fasta_files, rep_db, ecut = "1e-09") :
	...
	outopt=" -outfmt \"6 qseqid qseq qstart qend sseqid sseq sstart send sstrand sframe \
	length pident nident ppos gaps gapopen evalue bitscore qcovs qcovhsp qcovus\" "
	...
	if type_ome == "G" :
		...
		for species in regions :
			...
			for contig in regions[species] :
				...
				for ome in dbase :
					print quer, ome
					outfile = contig + "vs" + str(ome.split('/')[-1]) + "." + \
					cmd_blast + ".csv"
					cmd = cmd_blast + " -query " + quer + " -db " + ome + " " \
					+ outopt + " -evalue " + ecut + " -out " + outfile + \
					" -num_threads 32" 
					os.system(cmd)
				os.system("rm " + quer)
	#pour T ou P
	if type_ome == "T" or type_ome == "P" :
		...
		for species in regions :
			...
			for contig in regions[species] :
				...
				for ome in dbase :
					#print quer, ome
					outfile = contig.replace("|","_") + "vs" + \
					str(ome.split('/')[-1]) + "." + cmd_blast + ".csv"
					#cmd = cmd_blast + " -query " + quer + " -db " + ome \
					#+ " " + outopt + " -out " + outfile + " -num_threads 96"
					cmd = cmd_blast + " -query " + quer + " -db " + ome + " " \
					+ outopt + " -evalue " + ecut + " -out " + outfile +\
					 " -num_threads 32" 
					os.system(cmd)
...
#Notes & Commentaires
"""
- /data/mortaza/src/OPRvsgenome/db_building_part2.py
